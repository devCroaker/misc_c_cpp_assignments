...
Trainer::Trainer(Settings const& settings, Network* pNetwork)
	: m_pNetwork(pNetwork)
...
void Trainer::Train(TrainingData const& trainingData)
{
	...
	while ((m_trainingSetAccuracy < m_desiredAccuracy || m_generalizationSetAccuracy < m_desiredAccuracy) && m_currentEpoch < m_maxEpochs)
	{
		...
		std::cout << "Epoch: "  << std::left << std::setw(3) << m_currentEpoch;
		std::cout << " Training Set Accuracy: " << std::right << std::setw(10) << m_trainingSetAccuracy << "%, MSE: " << std::setw(10) << m_trainingSetMSE;
		std::cout << " | Generalization Set Accuracy: " << std::right << std::setw(10) << m_generalizationSetAccuracy << "%, MSE: " << std::setw(10) << m_generalizationSetMSE << std::endl;
...
double Trainer::GetHiddenErrorGradient(int32_t hiddenIdx) const
{
...
void Trainer::RunEpoch(TrainingSet const& trainingSet)
{
...
void Trainer::Backpropagate(std::vector<int32_t> const& expectedOutputs)
{
...
void Trainer::GetSetAccuracyAndMSE(TrainingSet const& trainingSet, double& accuracy, double& MSE) const
{
	accuracy = 0;
	MSE = 0;
